...
class TestPluginLoader < Test::Unit::TestCase
  ...
  def test_should_find_all_availble_plugins_and_return_as_all_plugins
    assert_plugins [ :engine, :stubby, :plugin_with_no_lib_dir, :gemlike, :acts_as_chunky_bacon, :a, defined?(Rails2Ruby2) ? :rails2_ruby2 : nil].compact, @loader.all_plugins.reverse, @failure_tip
  end

  def test_should_return_all_plugins_as_plugins_when_registered_plugin_list_is_untouched
    assert_plugins [:a, :acts_as_chunky_bacon, :engine, :gemlike, :plugin_with_no_lib_dir, defined?(Rails2Ruby2) ? :rails2_ruby2 : nil, :stubby].compact, @loader.plugins, @failure_tip
  end
  ...
  def test_should_load_specified_plugins_in_order_and_then_all_remaining_plugins_when_all_is_used
    ...
    assert_plugins [:stubby, :acts_as_chunky_bacon, :a, :engine, :gemlike, :plugin_with_no_lib_dir, defined?(Rails2Ruby2) ? :rails2_ruby2 : nil].compact, @loader.plugins, @failure_tip
  end
  ...
  def test_should_be_able_to_specify_loading_of_plugins_loaded_after_all
    ...
    assert_plugins [:stubby, :a, :engine, :gemlike, :plugin_with_no_lib_dir, defined?(Rails2Ruby2) ? :rails2_ruby2 : nil, :acts_as_chunky_bacon].compact, @loader.plugins, @failure_tip
  end
